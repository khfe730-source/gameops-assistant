...
from mock_data.generators import incident
...
def get_ccu(scenario: Scenario, seed: int, ts: datetime) -> int:
    ...
    match scenario:
        case Scenario.INCIDENT_CCU_SPIKE:
            return incident.ccu_spike(seed, ts)
        case _:
            return metrics_generator.generate_ccu(seed, ts)
...
def get_matchmaking_queue(scenario: Scenario, seed: int, ts: datetime) -> dict:
    ...
    match scenario:
        case Scenario.INCIDENT_CCU_SPIKE:
            return incident.queue_overloaded(seed, ts)
        case Scenario.INCIDENT_QUEUE_STUCK:
            return incident.queue_stuck(seed, ts)
        case _:
            return metrics_generator.generate_matchmaking_queue(seed, ts)
...
def get_error_rate(scenario: Scenario, seed: int, ts: datetime) -> dict:
    ...
    match scenario:
        case Scenario.INCIDENT_ERROR_SPIKE:
            return incident.error_spike(seed, ts)
        case Scenario.INCIDENT_CCU_SPIKE:
            return incident.error_elevated(seed, ts)
        case _:
            return metrics_generator.generate_error_rate(seed, ts)
...
def get_latency(scenario: Scenario, seed: int, ts: datetime) -> dict:
    ...
    match scenario:
        case Scenario.INCIDENT_ZONE_LATENCY:
            return incident.zone_latency_spike(seed, ts)
        case _:
            return metrics_generator.generate_latency(seed, ts)
